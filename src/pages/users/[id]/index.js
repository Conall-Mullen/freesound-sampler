...
export default function User() {
  ...
  const updateAllSamples = useSamplerStore((state) => state.updateAllSamples);
  const updateAllSamplesVolume = useSamplerStore(
    (state) => state.updateAllSamplesVolume
  );
      ...
      <ul>
        {patches.map((patch, index) => (
          <li key={index} className="patch-list-item">
            <p
              onClick={() => {
                updateAllSamples(patches[index].audioSources);
                updateAllSamplesVolume(patches[index].faderVolume);
